...
def listen_to_udp():
    ...
    while True:
        ...
        # get the packet from the listener and decode it using the telemetry_f1_2021 package
        ...
        if data.__contains__("m_lap_data"):
            ...
            player_json = data_json.get("m_lap_data")[data_json.get(
        ...
        if (data.__contains__("m_car_status_data")):
            data_json = json.loads(data)
            # packets_json.write(str(data_json.get("m_car_status_data")[data_json.get("m_header").get("m_player_car_index")]).replace("\'", "\"") + ',\n')
            player_json = data_json.get("m_car_status_data")[
                data_json.get("m_header").get("m_player_car_index")]
        ...
        if (data.__contains__('m_car_telemetry_data')):
            data_json = json.loads(data)
            player_json = data_json.get("m_car_telemetry_data")[
                data_json.get("m_header").get("m_player_car_index")]
            if is_tracking and player_car is not None:
